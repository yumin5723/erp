...
class ImportCityController extends Controller{
    ...
    /**
     * action for import area date
     * @return [type] [description]
     */
    public function actionRun(){
    	$provData = json_decode($this->_provData,true);
        $citydata = json_decode($this->_cityData,true);
        foreach($provData as $value){
            $model = new City;
            $model->pid = 0;
        ...
        foreach($citydata as $value){
            $model = new City;
            $model->pid = $value['ProID'];
            $model->name = $value['name'];
            $model->save();
        }
        echo "area data has been imported success!";
    }
}