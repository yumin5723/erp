...
class ResizeImage {
    public function getThumb($file_path,$dir,$ext,$width=100,$height=100,$thumbname=false){
        ...
        // $this->resize($file_path,$show_pic_scal[0],$show_pic_scal[1],$thumbname); //成比例缩减
        $this->resize($file_path,$width,$height,$thumbname); //固定大小
        if($thumbname){
    ...
    * **/    
    ...
    {     
        ...
        $temp_w=$w;//intval($width*$per);//计算原图缩放后的宽度 
        $temp_h=intval($height*$per);//计算原图缩放后的高度
        $dst_x = 0;     
        if($per1>1){
            $temp_h = $h;
            $temp_w = intval($h*$per1);
            $dst_x = ($temp_w/2)-($w/2);  
        }
        $temp_img=imagecreatetruecolor($temp_w,$temp_h);//创建画布     
        $im=$this->create($src);    
        imagecopyresampled($temp_img,$im,0,0,0,0,$temp_w,$temp_h,$width,$height); 
        // imagecopyresampled($temp_img,$im,0,0,0,0,$temp_w,$temp_h,$width,$width);  
        if($per1>$per2)     
        {     
            imagecopyresampled($temp_img,$im,0,0,$dst_x,0,$temp_w,$temp_h,$width,$height);   
        ...
        if($per1<$per2)     
        {     
            imagecopyresampled($temp_img,$im,0,0,$dst_x,0,$temp_w,$temp_h,$width,$width); 
            imagejpeg($temp_img,$savepath, 100);     
            imagedestroy($im);
